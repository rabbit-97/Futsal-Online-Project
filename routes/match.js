...
router.post("/match", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // 웹소켓을 통해 대기열 알림 전송
    io.emit("newMatchRequest", { userId });
...
router.get("/status", authMiddleware, async (req, res) => {
  ...
  try {
    const userStatus = await prisma.matchRequest.findMany({
      where: {
        requesterUserId: token,
      },
      include: {
    ...
    res.status(200).json({ message: "유저 상태 조회 성공", userStatus });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "유저 상태 조회 중 오류가 발생했습니다." });
  }
...
// 모든 매칭 요청 삭제 api
router.delete("/status/delete", authMiddleware, async (req, res) => {
  const token = req.user.userId;

  try {
    const deletedRequests = await prisma.matchRequest.deleteMany({
      where: { requesterUserId: token },
    });

    res.status(200).json({
      message: "모든 매칭 요청이 삭제되었습니다.",
